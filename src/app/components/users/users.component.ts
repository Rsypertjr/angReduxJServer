...
export class Users {  
    ...
    ngOnInit(){
        ...
        this.svc.getUsers()
                .then((users:User[]) => {
                    this.apiUsers = users;
                    //alert(JSON.stringify(this.apiUsers));  
                    for(var i=0;i<this.apiUsers.length;i++)
                        {
                             //alert(JSON.stringify(this.apiUsers[i]));
                             this.store.dispatch(this.userActions.addUserSuccess(this.apiUsers[i])); 
                        }                 
            });      
